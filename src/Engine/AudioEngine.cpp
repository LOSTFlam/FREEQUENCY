...
namespace freequency::engine
{
    ...
    namespace
    {
        ...
        */
        ...
        /** Basic overlap-add (OLA) time-stretch — changes length, preserves pitch.
            Not transient-aware, but a real, usable warp; runs on the message
            thread when building the clip snapshot. */
        std::unique_ptr<juce::AudioBuffer<float>> olaTimeStretch (const juce::AudioBuffer<float>& in,
                                                                  double stretch)
        {
        ...
        /** Resample a buffer by `ratio` (output length = input/ratio). */
        std::unique_ptr<juce::AudioBuffer<float>> resampleByRatio (const juce::AudioBuffer<float>& in,
                                                                   double ratio)
        {
        ...
        /** Apply warp (stretch) + pitch shift to a clip's buffer in place. */
        std::unique_ptr<juce::AudioBuffer<float>> applyWarp (std::unique_ptr<juce::AudioBuffer<float>> buf,
                                                            double stretchRatio, int pitchSemitones)
        {
            if (buf == nullptr)
                return buf;

            const double pitchFactor = std::pow (2.0, pitchSemitones / 12.0);
            const double totalStretch = stretchRatio * pitchFactor;

            if (std::abs (totalStretch - 1.0) > 1.0e-4)
                buf = olaTimeStretch (*buf, totalStretch);   // length *= stretch*pitchFactor

            if (std::abs (pitchFactor - 1.0) > 1.0e-4)
                buf = resampleByRatio (*buf, pitchFactor);   // length /= pitchFactor, pitch *= pitchFactor
        ...
        for (int i = 0; i < timeline.getNumTracks(); ++i)
        {
            ...
            else if (auto* audioTrack = dynamic_cast<models::AudioTrack*> (track))
            {
                ...
                for (int c = 0; c < audioTrack->getNumClips(); ++c)
                {
                    ...
                    buffer = applyWarp (std::move (buffer), clip->stretchRatio, clip->pitchSemitones);
